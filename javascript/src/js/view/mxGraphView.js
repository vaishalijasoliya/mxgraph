 ...
 *
 * Calls <validateBackgroundImages>.
 */
mxGraphView.prototype.validateBackgrounds = function () {
  this.validateBackgroundImages();
  this.validateBackgroundPage();
};

/**
 * Function: validateBackgroundImages
 *
 * Validates the background images.
 */
mxGraphView.prototype.validateBackgroundImages = function () {
  var bgs = this.graph.getBackgroundImages();

  if (this.backgroundImages == undefined) {
    this.backgroundImages = [];
  }

  // Remove old backgroundImages
  if (this.backgroundImages.length > bgs.length) {
    for (
      var start = bgs.length;
      start < this.backgroundImages.length;
      start++
    ) {
      if (this.backgroundImages[start] != null) {
        this.backgroundImages[start].destroy();
        this.backgroundImages[start] = null;
      }
    }
  }
    ...
    for (var i = 0; i < bgs.length; i++) {
      ...
      if (bg != null) {
        if (
          ...
          this.backgroundImages[i].image != bg.src
        ) {
          var bounds = new mxRectangle(0, 0, 1, 1);

          this.backgroundImages[i] = new mxImageShape(bounds, bg.src);
          this.backgroundImages[i].dialect = this.graph.dialect;
          this.backgroundImages[i].init(this.backgroundPane);
          this.backgroundImages[i].redraw();

          // Workaround for ignored event on background in IE8 standards mode
          if (document.documentMode == 8 && !mxClient.IS_EM) {
            mxEvent.addGestureListeners(
              this.backgroundImage[i].node,
              mxUtils.bind(this, function (evt) {
                this.graph.fireMouseEvent(
                  mxEvent.MOUSE_DOWN,
                  new mxMouseEvent(evt)
        ...
        this.redrawBackgroundImage(this.backgroundImages[i], bg);
      } else if (this.backgroundImages[i] != null) {
        this.backgroundImages[i].destroy();
        this.backgroundImages[i] = null;
      }
    }
  }
 ...
 * {
 *   backgroundImage.bounds.x = bg.x;
 *   backgroundImage.bounds.y = bg.y;
...
mxGraphView.prototype.redrawBackgroundImage = function (backgroundImage, bg) {
  ...
  backgroundImage.bounds.x = this.scale * bg.x;
  backgroundImage.bounds.y = this.scale * bg.y;
